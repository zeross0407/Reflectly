...
class _EntriesPageState extends State<EntriesPage>
  ...
  void initState() {
    ...
    //repository = Repository<int, Entry>(name: 'entry_box');

    //_loadData(); // Tải dữ liệu khi widget khởi tạo
  ...
  //   await repository.init(); // Mở hộp
  //   items = await repository.getAll(); // Lấy tất cả dữ liệu

  //   for (int i = 0; i < items.length; i++) {
  //     entry_points.add(items[i]);
  //   }
  //   List<int> split = [if (!entry_points.isEmpty) 0];

  //   for (int i = 1; i < entry_points.length; i++) {
  //     if (!isSameDay(
  //         entry_points[i - 1].submitTime, entry_points[i].submitTime)) {
  ...
  //     entry_points.insert(
  //         split[i] + count, entry_points[split[i] + count].submitTime);
  //     count++;
  //   }
  ...
  Widget build(BuildContext context) {
    ...
    return ChangeNotifierProvider<Entries_Viewmodel>(
      create: (context) => Entries_Viewmodel(),
      child: Consumer<Entries_Viewmodel>(
